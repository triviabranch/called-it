...
async function loadRooms() {
  try {
    ...
    roomsTable.innerHTML = liveRooms.map(room => `<tr><td><strong>${esc(roomLabel(room))}</strong><small>${esc(room.fixture?.competition || room.fixture?.venue || "Live fixture")}</small></td><td>${esc(String(room.session?.status || "—").toUpperCase())}</td><td>${room.playerCount || 0}</td><td>${(room.calls || []).length}</td><td><button class="table-action" data-room="${esc(room.roomId)}">View room</button></td></tr>`).join("") || '<tr><td colspan="5">No live rooms are currently registered.</td></tr>';
