...
async function liveFixtures(env) {
  ...
  const staleIndex = response.status === 404 || nowForIndex - Number(data.fetchedAt || 0) > 15000;
  const nearKickoff = (data.fixtures || []).some(item => {
    const kickoff = new Date(item.date || 0).getTime();
    return item.state === "pre" && Number.isFinite(kickoff) && kickoff >= nowForIndex - 10 * 60 * 1000 && kickoff <= nowForIndex + 5 * 60 * 1000;
  });
  const staleSchema = data.fixtureIndexVersion !== 2 || data.windowMinutes !== 120 || !Array.isArray(data.enabledCompetitions);
  if (staleIndex || nearKickoff || staleSchema) { await refreshFixtureIndex(env); response = await env.FIXTURE_INDEX.get(id).fetch("https://fixture-index/fixtures"); data = await response.json(); }
...
export class MatchRoom {
  ...
  async webSocketMessage(ws, raw) {
    ...
    if (m.type === "join") { let p = this.room.players.find(x => x.id === m.playerId), isNewPlayer = !p; if (!p) { p = { id: crypto.randomUUID(), name: String(m.name || "Supporter").slice(0,20), points: 0, rounds: 0, calls: 0, correct: 0 }; this.room.players.push(p); } else if (m.name) p.name = String(m.name).slice(0,20); if (isNewPlayer && (this.room.mode === "live" || (this.room.fixture?.id && this.room.mode !== "simulation"))) { await this.refreshLive(true); this.settlePreMatch(this.room.session.clock || 0); } this.room.playerPreMatch ||= {}; if (this.room.fixture?.state === "in") { const current = this.buildPreMatch(true, p.id), hasRelevantEvent = this.room.timeline.some(e => ["goal", "goal-kick", "foul"].includes(e.type)); if (!this.room.playerPreMatch[p.id] && hasRelevantEvent) this.room.playerPreMatch[p.id] = current; else if (this.room.playerPreMatch[p.id]) { this.room.playerPreMatch[p.id].forEach((oldQuestion, index) => { const nextQuestion = current[index]; if (oldQuestion && nextQuestion && !oldQuestion.settled && nextQuestion.type.startsWith("next-")) Object.assign(oldQuestion, { type: nextQuestion.type, question: nextQuestion.question, input: nextQuestion.input, choices: nextQuestion.choices, afterOffset: nextQuestion.afterOffset }); }); } } if (this.room.mode === "live" && this.room.session.status === "lobby") { this.room.session = { status: "running", startedAt: Date.now(), round: null, clock: 0, nextRoundIndex: 0, nextQuestionAt: nextLiveCallAt(this.room.fixture), liveScheduleKickoff: Date.parse(this.room.fixture?.date), mode: "live", speed: 1, lastQuestionType: null }; } this.rebuildLeaderboard(); ws.send(JSON.stringify({ type:"identity", playerId:p.id })); }
