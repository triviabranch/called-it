let ws, roomId, state, playerId, role, submittedRoundId = null, leaderboardOpen = false, callsOpen = false, joinModalOpen = false, joinModalDismissed = false;
...
function committedCallsModal() {
  ...
  const playerHtml = players.map(player => {
    const calls = (player.calls || []).map(call => `<div class="call-row"><span>${esc(call.question)}</span><small>${esc(call.answer)}</small><span class="call-status ${String(call.status).toLowerCase()}">${esc(call.status)}</span></div>`).join("");
