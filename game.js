...
function fixtureCard(f) {
  ...
  const score = live ? `${f.home.score ?? "–"} — ${f.away.score ?? "–"}` : "v";
  return `<article class="fixture-choice" data-fixture="${esc(f.id)}" data-sport="${esc(f.sport || "soccer")}" data-league="${esc(f.league || "eng.1")}"><button class="fixture-open" type="button"><span class="fixture-teams"><strong>${esc(f.home.name)}</strong><b>${score}</b><strong>${esc(f.away.name)}</strong></span><small class="fixture-status ${live ? "live" : ""}">${esc(live ? "LIVE NOW" : scheduledTime(f.date))} · ${esc(competitionName(f))} · ${esc(broadcastName(f))} ${f.venue ? ` · ${esc(f.venue)}` : ""}</small></button>${live ? "" : `<button class="reminder" type="button" data-reminder="${esc(f.id)}" aria-label="Add ${esc(f.home.name)} v ${esc(f.away.name)} to calendar" title="Add to calendar"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 3v3m10-3v3M4.5 9.5h15M6 5h12a2 2 0 0 1 2 2v11.5a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z"/></svg></button>`}</article>`;
}
...
function showJoinModal(fixture, league) {
  ...
  modal.innerHTML = `<section class="join-modal" role="dialog" aria-modal="true" aria-labelledby="join-modal-title"><button type="button" class="modal-close join-modal-close" aria-label="Close join dialog">×</button><div class="phase">${esc(liveLabel)} · ${esc(competitionName(fixture))}</div><h2 id="join-modal-title">${esc(fixture.home.name)} <i>v</i> ${esc(fixture.away.name)}</h2><p class="muted">Join this live match and make your calls as it unfolds.</p><label class="join-modal-name">Your name<input type="text" maxlength="20" autocomplete="name" value="${esc(pendingName)}" placeholder="Enter your name"></label><div class="join-modal-actions"><button type="button" class="join-confirm">Join game</button></div></section>`;
...
function render() {
  ...
  const playerJoinModal = !me && joinModalOpen ? `<div class="join-modal-backdrop player-join-modal-backdrop"><section class="join-modal" role="dialog" aria-modal="true" aria-labelledby="player-join-title"><button type="button" class="modal-close join-modal-close" data-player-join-close aria-label="Close join dialog">×</button><div class="phase">LIVE MATCHDAY</div><h2 id="player-join-title">${esc(f.home?.name)} <i>v</i> ${esc(f.away?.name)}</h2><p class="muted">Join this live match and make your calls as it unfolds.</p><label class="join-modal-name">Your name<input type="text" maxlength="20" autocomplete="name" value="${esc(pendingName)}" placeholder="Enter your name" data-player-join-name></label><div class="join-modal-actions"><button type="button" class="join-confirm" data-player-join-confirm>Join game</button></div></section></div>` : "";
  ...
  const call = callLocked ? '<div class="call-locked"><span class="lock-dot"></span><b>CALL LOCKED IN</b><span>Stay with the match — the next call will land here.</span></div>' : me && r?.status === "voting" ? `<div class="call-modal-backdrop"><section class="call-modal" role="dialog" aria-label="Make your call"><div class="live-kicker"><span class="live-dot"></span>FIXTURE CALL · ALL PLAYERS <span class="countdown" data-vote-countdown data-vote-ends-at="${r.voteEndsAt || 0}">${voteLabel}</span></div><h2>${esc(r.question)}</h2><p>Make your call. It stays open until the match answers it.</p><div class="answers">${(r.choices || []).map((a, index) => `<button class="answer" data-answer="${esc(a.key)}"><span class="answer-index">${String.fromCharCode(65 + index)}</span>${esc(a.label)}</button>`).join("")}</div></section></div>` : '';
  const matchFinished = state.session?.status === "complete";
  const leaderboard = `<div class="leaderboard-modal-backdrop" data-leaderboard-close><section class="leaderboard-modal" role="dialog" aria-modal="true" aria-label="Match leaderboard"><div class="section-head"><h2>${matchFinished ? "Final results" : "Leaderboard"}</h2><button class="modal-close" data-leaderboard-close aria-label="Close leaderboard">×</button></div><div class="leaderboard-match"><div class="phase">${matchFinished ? "FULL TIME" : "LIVE MATCH"} · ${esc(competitionName(f))}</div><h3 class="scoreboard-teams leaderboard-scoreboard"><span><b>${esc(f.home?.name)}</b><strong>${f.home?.score ?? "–"}</strong></span><span><b>${esc(f.away?.name)}</b><strong>${f.away?.score ?? "–"}</strong></span></h3></div><p class="muted">Room results</p><div class="leaders">${(state.leaderboard || []).map(p => `<div><b>#${p.rank} ${esc(p.name)}</b><span>${p.points} pts · ${p.rounds} calls</span></div>`).join("") || '<p class="muted">No players yet.</p>'}</div></section></div>`;
  ...
  const playerHtml = players.map(player => {
    const calls = (player.calls || []).map(call => {
      ...
      const statusMark = status === "committed" || status === "correct"
        ? `<span class="call-status call-status-icon ${status}" title="${esc(call.status)}" aria-label="${esc(call.status)}"><img src="assets/called-it-logo.png" alt="${esc(call.status)}"></span>`
