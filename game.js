...
function committedCallsModal() {
  ...
  app.innerHTML = `<header class="brand"><img src="assets/called-it-wordmark.png" alt="Called It"><span class="room">${esc(state.session?.status || "LIVE")}</span></header><div class="eyebrow-row"><div class="eyebrow">LIVE MATCHDAY</div><button class="leaderboard-trigger" data-leaderboard-open>Leaderboard <span>↗</span></button></div><div class="match-header ${isHalfTime ? "at-half-time" : ""}"><h1>${esc(f.home?.name)} <i>${f.home?.score ?? "–"} — ${f.away?.score ?? "–"}</i> ${esc(f.away?.name)}</h1><p><strong class="match-status">${esc(matchStatus)}</strong> <span>·</span> match clock <b>${clock(state.session?.clock, state.session?.clockDisplay)}</b></p></div>${playerStats}${callsModal}<section class="card broadcast-feed" aria-live="polite"><div class="section-head"><h2>Match feed</h2><span class="feed-live"><span class="live-dot"></span>LIVE</span></div><div class="broadcast-feed-list">${feed}</div></section><footer class="called-it-footer"><img src="assets/called-it-wordmark.png" alt="Called It"><span>Live matchday play-along</span></footer>${simulationPanel()}${state.session?.status === "lobby" ? `<section class="card"><h2>Join this match</h2>${join}<p class="muted">${state.players?.length || 0} supporter(s) are in the room.</p></section>` : ""}${me && incomplete ? preMatchCard(me) : ""}${me && !incomplete ? call : ""}${leaderboardOpen ? leaderboard : ""}`;
