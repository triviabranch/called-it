...
async function joinFixture(fixture, league, name = "") {
  ...
  try { const response = await fetch("/api/room/fixture", { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({ fixture, sport: fixture.sport || "soccer", league, mode:"live" }) }); const data = await response.json().catch(() => ({})); if (!response.ok || !data.roomId) throw Error(data.error || "Could not open fixture room"); location.href = `/play/${encodeURIComponent(data.roomId)}`; } catch (error) { if (button) button.disabled = false; alert(error.message); }
}
