...
function showJoinModal(fixture, league) {
  ...
  modal.innerHTML = `<section class="join-modal" role="dialog" aria-modal="true" aria-labelledby="join-modal-title"><div class="phase">${esc(liveLabel)} · ${esc(competitionName(fixture))}</div><h2 id="join-modal-title">${esc(fixture.home.name)} <i>v</i> ${esc(fixture.away.name)}</h2><p class="muted">Join this live match and make your calls as it unfolds.</p><div class="join-modal-actions"><button type="button" class="join-cancel">Not now</button><button type="button" class="join-confirm">Join game</button></div></section>`;
  document.body.appendChild(modal);
  const close = () => modal.remove();
  modal.querySelector(".join-cancel").onclick = close;
  modal.addEventListener("click", event => { if (event.target === modal) close(); });
  modal.querySelector(".join-confirm").onclick = () => { close(); joinFixture(fixture, league); };
  const onKey = event => { if (event.key === "Escape") { close(); document.removeEventListener("keydown", onKey); } };
  document.addEventListener("keydown", onKey);
  modal.querySelector(".join-confirm").focus();
}
