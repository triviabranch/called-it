...
function render() {
  ...
  const now = Date.now(), remaining = r?.status === "voting" && r.voteEndsAt ? Math.max(0, Math.ceil((r.voteEndsAt - now) / 1000)) : 0, voteLabel = r?.status === "voting" && r.voteEndsAt && state.session?.mode === "simulation" ? `${remaining}s` : "", nextCallSeconds = state.session?.nextQuestionAt ? Math.max(0, Math.ceil((state.session.nextQuestionAt - now) / 1000)) : null, nextCallLabel = r?.status === "voting" ? "" : nextCallSeconds == null ? "" : nextCallSeconds <= 0 ? "" : `${String(Math.floor(nextCallSeconds / 60)).padStart(2, "0")}:${String(nextCallSeconds % 60).padStart(2, "0")}`;
