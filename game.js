...
function connect() { ws = new WebSocket(`${location.protocol === "https:" ? "wss" : "ws"}://${location.host}/api/room/${roomId}`); ws.onopen = () => { lastStateReceivedAt = Date.now(); pendingJoinSent = false; attemptPendingJoin(); setTimeout(attemptPendingJoin, 250); }; ws.onmessage = event => { const message = JSON.parse(event.data); if (message.type === "identity") { playerId = message.playerId; localStorage.setItem(`calledItPlayer:${roomId}`, playerId); if (message.playerToken) localStorage.setItem(`calledItPlayerToken:${roomId}`, message.playerToken); attemptPendingJoin(); } if (message.state) { lastStateReceivedAt = Date.now(); state = message.state; if (state.fixture?.id) localStorage.setItem(`calledItRoom:${state.fixture.id}`, String(roomId)); attemptPendingJoin(); render(); setTimeout(attemptPendingJoin, 0); } }; ws.onclose = () => { if (state?.session?.status === "complete") return; setTimeout(connect, 1500); }; }
...
function render() {
  ...
  const goalLines = team => liveEvents.filter(event => event.type === "goal" && event.valid !== false && !event.invalidated && normaliseName(event.team) === normaliseName(team)).map(event => {
...
function scorecardGoalLines(team) {
  ...
  return (state.timeline || []).filter(event => event.type === "goal" && event.valid !== false && !event.invalidated && normalise(event.team) === normalise(team)).sort((a, b) => (a.offset || 0) - (b.offset || 0)).map(event => {
    const scorer = event.athletes?.[0] || "";
