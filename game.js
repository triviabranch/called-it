...
function render() {
  ...
  const leaderboard = `<div class="leaderboard-modal-backdrop" data-leaderboard-close><section class="leaderboard-modal" role="dialog" aria-modal="true" aria-label="Match leaderboard"><div class="section-head"><h2>${matchFinished ? "Final results" : "Leaderboard"}<button class="modal-close" data-leaderboard-close aria-label="Close leaderboard">×</button></div><div class="leaderboard-match"><div class="phase">${matchFinished ? "FULL TIME" : "LIVE MATCH"} · ${esc(competitionName(f))}</div><h3>${esc(f.home?.name)} <strong>${f.home?.score ?? "–"} — ${f.away?.score ?? "–"}</strong> ${esc(f.away?.name)}</h3><small>${esc(f.name || `${f.home?.name} v ${f.away?.name}`)}</small></div><p class="muted">Room results</p><div class="leaders">${(state.leaderboard || []).map(p => `<div><b>#${p.rank} ${esc(p.name)}</b><span>${p.points} pts · ${p.rounds} calls</span></div>`).join("") || '<p class="muted">No players yet.</p>'}</div></section></div>`;
