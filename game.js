...
setInterval(() => {
  ...
  const vote = document.querySelector("[data-vote-countdown]");
  if (vote) vote.textContent = `${Math.max(0, Math.ceil((Number(vote.dataset.voteEndsAt) - now) / 1000))}s`;
}, 1000);

async function getFixtures(region = "") {
  const query = region ? "?region=" + encodeURIComponent(region) : "";
  const response = await fetch("/api/live-fixtures" + query);
...
function fixtureCard(f) {
  const live = f.state === "in", completed = Boolean(f.completed || f.state === "post");
  const score = live || completed ? String(f.home.score ?? "–") + " — " + String(f.away.score ?? "–") : "v";
  const label = completed ? "FULL TIME" : live ? "LIVE NOW" : scheduledTime(f.date);
  const broadcast = completed ? "" : " · " + esc(broadcastName(f)) + (f.venue ? " · " + esc(f.venue) : "");
  const reminder = live || completed ? "" : '<button class="reminder" type="button" data-reminder="' + esc(f.id) + '" aria-label="Add ' + esc(f.home.name) + ' v ' + esc(f.away.name) + ' to calendar" title="Add to calendar"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 3v3m10-3v3M4.5 9.5h15M6 5h12a2 2 0 0 1 2 2v11.5a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z"/></svg></button>';
  return '<article class="fixture-choice" data-fixture="' + esc(f.id) + '" data-sport="' + esc(f.sport || "soccer") + '" data-league="' + esc(f.league || "eng.1") + '" data-completed="' + (completed ? "true" : "false") + '"><button class="fixture-open" type="button"><span class="fixture-teams"><strong>' + esc(f.home.name) + '</strong><b>' + score + '</b><strong>' + esc(f.away.name) + '</strong></span><small class="fixture-status ' + (live ? "live" : "") + ' ' + (completed ? "completed" : "") + '">' + esc(label) + ' · ' + esc(competitionName(f)) + broadcast + '</small></button>' + reminder + '</article>';
}
...
async function bootLanding() {
  app.innerHTML = '<header class="brand"><a href="/" aria-label="Called It home"><img src="assets/called-it-wordmark.png" alt="Called It"></a><span class="room">LIVE MATCHDAY</span></header><h1>Pick a fixture. <i>Call it live.</i></h1><section class="card"><div class="section-head"><h2 class="fixture-list-title">Live now</h2><div class="fixture-list-actions"><label class="fixture-region"><span>TV in</span><select data-fixture-region aria-label="TV country"><option value="gb" aria-label="United Kingdom" title="United Kingdom">🇬🇧</option><option value="au" aria-label="Australia" title="Australia">🇦🇺</option><option value="us" aria-label="United States" title="United States">🇺🇸</option><option value="ca" aria-label="Canada" title="Canada">🇨🇦</option><option value="nz" aria-label="New Zealand" title="New Zealand">🇳🇿</option><option value="ie" aria-label="Ireland" title="Ireland">🇮🇪</option></select></label><span class="live-list-status"><span class="live-dot"></span>LIVE</span><button type="button" class="fixture-refresh" data-fixture-refresh aria-label="Refresh fixture list" title="Refresh fixture list">↻</button></div></div><div id="fixtures"><p class="muted">Loading this week’s live games…</p></div></section>';
  async function load(regionOverride = "") { const target = document.querySelector("#fixtures"), refreshButton = document.querySelector("[data-fixture-refresh]"), regionSelect = document.querySelector("[data-fixture-region]"), savedRegion = localStorage.getItem("calledItFixtureRegion") || ""; let region = regionOverride || savedRegion; if (savedRegion && [...regionSelect.options].some(option => option.value === savedRegion)) regionSelect.value = savedRegion; refreshButton?.classList.add("is-refreshing"); target.innerHTML = '<p class="muted">Checking the fixture list…</p>'; try { const data = await getFixtures(region); if (Array.isArray(data.enabledRegions)) { const enabled = new Set(data.enabledRegions); const available = regionOptions.filter(([code]) => enabled.has(code)); regionSelect.innerHTML = available.map(([code, label, flag]) => `<option value="${code}" aria-label="${label}" title="${label}">${flag}</option>`).join(""); if (!enabled.has(region)) { region = available[0]?.[0] || "gb"; localStorage.setItem("calledItFixtureRegion", region); } regionSelect.value = region; } if (!savedRegion && data.region) regionSelect.value = data.region; const activeFixtures = data.fixtures || []; const completedFixtures = data.completedFixtures || []; const available = [...activeFixtures, ...completedFixtures]; const liveCount = available.filter(f => f.state === "in").length; document.querySelector(".fixture-list-title").textContent = liveCount ? "Live now" : "Upcoming"; document.querySelector(".live-list-status")?.toggleAttribute("hidden", !liveCount); if (!available.length) { target.innerHTML = '<p class="muted">There are no supported fixtures live or scheduled today.</p>'; return; } const live = available.filter(f => f.state === "in"), upcoming = available.filter(f => f.state === "pre"), completed = available.filter(f => f.completed || f.state === "post"), hierarchy = ["eng.1", "eng.2", "sco.1", "esp.1", "ger.1", "ita.1", "fra.1", "usa.1", "aus.1", "3"], competitions = [...new Map(available.map(f => [competitionName(f), f.competition || f.league])).entries()].sort((a, b) => (hierarchy.indexOf(a[1]) === -1 ? 999 : hierarchy.indexOf(a[1])) - (hierarchy.indexOf(b[1]) === -1 ? 999 : hierarchy.indexOf(b[1]))); const section = (title, fixtures, empty) => fixtures.length ? `<section class="fixture-group"><h3>${title}</h3>${fixtures.map(fixtureCard).join("")}</section>` : (empty ? `<section class="fixture-group"><h3>${title}</h3><p class="muted">${empty}</p></section>` : ""); target.innerHTML = `<div class="comp-filters"><button class="selected" data-comp-filter="">All competitions</button>${competitions.map(([label, key]) => `<button data-comp-filter="${esc(key)}">${esc(label)}</button>`).join("")}</div><div id="fixture-list">${section("Live now", live)}${section("Upcoming", upcoming, "No upcoming supported fixtures.")}${section("Full time", completed)}</div>`; const renderList = filter => { document.querySelectorAll("#fixture-list [data-fixture]").forEach(card => { const f = available.find(item => item.id === card.dataset.fixture); card.hidden = Boolean(filter && (f.competition || f.league) !== filter); card.querySelector(".fixture-open")?.addEventListener("click", () => f.completed || f.state === "post" ? showCompletedLeaderboard(f) : f.accessState === "locked" ? showOpeningModal(f) : showJoinModal(f, card.dataset.league)); card.querySelector("[data-reminder]")?.addEventListener("click", event => { event.stopPropagation(); addReminder(f); }); }); }; document.querySelectorAll("[data-comp-filter]").forEach(button => button.onclick = () => { document.querySelectorAll("[data-comp-filter]").forEach(item => item.classList.toggle("selected", item === button)); renderList(button.dataset.compFilter); }); renderList(""); } catch (error) { target.innerHTML = `<p class="error">${esc(error.message)}</p>`; } finally { refreshButton?.classList.remove("is-refreshing"); } }
  await load();
  document.querySelector("[data-fixture-refresh]")?.addEventListener("click", () => load());
  document.querySelector("[data-fixture-region]")?.addEventListener("change", event => {
    localStorage.setItem("calledItFixtureRegion", event.target.value);
...
function showOpeningModal(fixture) {
  document.querySelectorAll(".join-modal-backdrop").forEach(item => item.remove());
  const modal = document.createElement("div");
  modal.className = "join-modal-backdrop";
  const opensAt = fixture.opensAt ? new Intl.DateTimeFormat("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(fixture.opensAt)) : "later";
  modal.innerHTML = `<section class="join-modal" role="dialog" aria-modal="true" aria-labelledby="opening-modal-title"><a class="modal-brand" href="/" aria-label="Called It home"><img src="assets/called-it-wordmark.png" alt="Called It"></a><button type="button" class="modal-close join-modal-close" aria-label="Close dialog">×</button><div class="phase">COMING UP · ${esc(competitionName(fixture))}</div><h2 class="opening-fixture">${esc(fixture.home?.name || "Home")} <i>v</i> ${esc(fixture.away?.name || "Away")}</h2><p id="opening-modal-title" class="opening-time">Called It opens at <i>${esc(opensAt)}</i></p><p class="muted">The match is on the way. Add a reminder and we’ll be ready when it’s time to call it.</p><div class="join-modal-actions"><button type="button" class="join-confirm" data-opening-reminder>Add a reminder</button></div></section>`;
  document.body.appendChild(modal);
  const close = () => modal.remove();
  modal.querySelector(".join-modal-close").onclick = close;
  modal.addEventListener("click", event => { if (event.target === modal) close(); });
  modal.querySelector("[data-opening-reminder]").onclick = async () => {
    const reminder = modal.querySelector("[data-opening-reminder]");
    reminder.disabled = true;
    try { await addReminder(fixture); reminder.textContent = "Reminder added"; }
    catch (error) { reminder.disabled = false; alert(error.message || "Could not add reminder"); }
  };
}
function showJoinModal(fixture, league) {
  ...
  modal.innerHTML = `<section class="join-modal" role="dialog" aria-modal="true" aria-labelledby="join-modal-title"><a class="modal-brand" href="/" aria-label="Called It home"><img src="assets/called-it-wordmark.png" alt="Called It"></a><button type="button" class="modal-close join-modal-close" aria-label="Close join dialog">×</button><div class="phase">${esc(liveLabel)} · ${esc(competitionName(fixture))}</div><h2 id="join-modal-title">${esc(fixture.home.name)} <i>v</i> ${esc(fixture.away.name)}</h2><p class="muted">Join this live match and make your calls as it unfolds.</p><label class="join-modal-name">Your name<input type="text" maxlength="20" autocomplete="name" value="${esc(pendingName)}" placeholder="Enter your name"></label><div class="join-modal-actions"><button type="button" class="join-confirm">Join game</button></div></section>`;
  document.body.appendChild(modal);
  const close = () => modal.remove();
  modal.querySelector(".join-modal-close").onclick = close;
  modal.addEventListener("click", event => { if (event.target === modal) close(); });
  modal.querySelector(".join-confirm").onclick = () => {
    const name = modal.querySelector("input").value.trim();
    if (!name) { modal.querySelector("input").focus(); return; }
    close(); joinFixture(fixture, league, name);
  };
...
function preMatchCard(me) {
  ...
  if (!current) return '<div class="pre-complete">Your calls are in. Stay with the match — the next live question will appear here.</div>';
  const scorerGroups = current.type === "first-goalscorer" ? [...new Map((current.choices || []).map(choice => [choice.team || "Players", true])).keys()] : [];
  const answerUi = current.input ? `<label class="number-call">${esc(current.input.suffix)}<input id="preNumber" type="number" min="${current.input.min}" max="${current.input.max}" step="${current.input.step}" value="${current.input.value ?? current.input.min}"></label><button class="answer" data-pre-submit="${esc(current.id)}">Submit call</button>` : current.type === "first-goalscorer" ? scorerGroups.map(team => `<fieldset class="lineup-group"><legend>${esc(team)}</legend><div class="answers">${current.choices.filter(choice => (choice.team || "Players") === team).map(a => `<button class="answer" data-pre="${esc(a.key)}">${esc(a.label)}</button>`).join("")}</div></fieldset>`).join("") : `<div class="answers">${current.choices.map(a => `<button class="answer" data-pre="${esc(a.key)}">${esc(a.label)}</button>`).join("")}</div>`;
  return `<div class="pre-modal-backdrop"><div class="pre-modal"><a class="modal-brand" href="/" aria-label="Called It home"><img src="assets/called-it-wordmark.png" alt="Called It"></a><div class="section-head"><div class="phase">Your pre-match calls · ${answered.length + 1} of ${questions.length}</div><button type="button" class="modal-close" data-pre-close aria-label="Close pre-match calls">×</button></div><h2>${esc(current.question)}</h2><p class="muted">Make your call before kick-off.</p>${answerUi}<button type="button" class="secondary" data-pre-close>Join later</button></div></div>`;
}
...
function updateTransientClock() {
  if (state?.session) document.querySelectorAll("[data-match-clock]").forEach(matchClock => { matchClock.textContent = clock(state.session.clock, state.session.clockDisplay); });
...
function render() {
  ...
  const now = Date.now(), remaining = r?.status === "voting" && r.voteEndsAt ? Math.max(0, Math.ceil((r.voteEndsAt - now) / 1000)) : 0, voteLabel = r?.status === "voting" && r.voteEndsAt && state.session?.mode === "simulation" ? `${remaining}s` : "", nextCallSeconds = state.session?.nextQuestionAt ? Math.max(0, Math.ceil((state.session.nextQuestionAt - now) / 1000)) : null, nextCallLabel = nextCallSeconds == null ? "" : nextCallSeconds <= 0 ? "" : `${String(Math.floor(nextCallSeconds / 60)).padStart(2, "0")}:${String(nextCallSeconds % 60).padStart(2, "0")}`;
  if (!me && !joinModalDismissed) joinModalOpen = true;
  const pendingName = state.fixture?.id ? (localStorage.getItem(`calledItPendingName:${state.fixture.id}`) || "") : "";
  const joinCta = !me ? `<section class="join-later"><span>Want to play along?</span><button type="button" data-player-join-open>Join this match</button></section>` : "";
  const playerJoinModal = !me && joinModalOpen ? `<div class="join-modal-backdrop player-join-modal-backdrop"><section class="join-modal" role="dialog" aria-modal="true" aria-labelledby="player-join-title"><a class="modal-brand" href="/" aria-label="Called It home"><img src="assets/called-it-wordmark.png" alt="Called It"></a><button type="button" class="modal-close join-modal-close" data-player-join-close aria-label="Close join dialog">×</button><div class="phase">LIVE MATCHDAY</div><h2 id="player-join-title">${esc(f.home?.name)} <i>v</i> ${esc(f.away?.name)}</h2><p class="muted">Join this live match and make your calls as it unfolds.</p><label class="join-modal-name">Your name<input type="text" maxlength="20" autocomplete="name" value="${esc(pendingName)}" placeholder="Enter your name" data-player-join-name></label><div class="join-modal-actions"><button type="button" class="join-confirm" data-player-join-confirm>Join game</button></div></section></div>` : "";
  const myCommittedCalls = state.committedCalls?.find(player => String(player.id) === String(playerId))?.calls || [];
...
function committedCallsModal() {
  ...
  app.innerHTML = `<header class="brand"><a href="/" aria-label="Called It home"><img src="assets/called-it-wordmark.png" alt="Called It"></a><span class="room">${esc(state.session?.status || "LIVE")}</span></header><div class="match-header ${isHalfTime ? "at-half-time" : ""}"><div class="scoreboard-live-marker">${esc(matchStatus)}</div><button class="match-header-leaderboard leaderboard-trigger" data-leaderboard-open>Leaderboard <span>↗</span></button><h1 class="scoreboard-teams"><span><b>${esc(f.home?.name)}</b><strong>${f.home?.score ?? "–"}</strong><small class="scoreboard-goal-list">${goalLines(f.home?.name)}</small></span><span><b>${esc(f.away?.name)}</b><strong>${f.away?.score ?? "–"}</strong><small class="scoreboard-goal-list">${goalLines(f.away?.name)}</small></span></h1>${nextCallLabel ? `<p class="next-call-line"><span class="next-call-countdown" data-next-call-countdown data-next-call-at="${state.session.nextQuestionAt}">NEXT CALL IN <b>${nextCallLabel}</b></span></p>` : ""}</div>${playerStats}${callsModal}<section class="card broadcast-feed" aria-live="polite"><div class="section-head"><h2>Match feed</h2><span class="feed-match-clock"><b data-match-clock>${clock(state.session?.clock, state.session?.clockDisplay)}</b></span></div><div class="broadcast-feed-list">${feed}</div></section><footer class="called-it-footer"><a href="/" aria-label="Called It home"><img src="assets/called-it-wordmark.png" alt="Called It"></a><span>Live matchday play-along</span></footer>${simulationPanel()}${!me ? `${joinCta}<p class="muted join-count">${state.players?.length || 0} supporter(s) are in the room.</p>` : ""}${playerJoinModal}${me && incomplete && r?.status !== "voting" ? preMatchCard(me) : ""}${me ? call : ""}${(leaderboardOpen || (state.session?.status === "complete" && !finalLeaderboardDismissed)) ? leaderboard : ""}`;
