...
function showJoinModal(fixture, league) {
  ...
  modal.innerHTML = `<section class="join-modal" role="dialog" aria-modal="true" aria-labelledby="join-modal-title"><div class="phase">${esc(liveLabel)} · ${esc(competitionName(fixture))}</div><h2 id="join-modal-title">${esc(fixture.home.name)} <i>v</i> ${esc(fixture.away.name)}</h2><p class="muted">Join this live match and make your calls as it unfolds.</p><label class="join-modal-name">Your name<input type="text" maxlength="20" autocomplete="name" value="${esc(pendingName)}" placeholder="Enter your name"></label><div class="join-modal-actions"><button type="button" class="join-cancel">Not now</button><button type="button" class="join-confirm">Join game</button></div></section>`;
  document.body.appendChild(modal);
  const close = () => modal.remove();
  modal.querySelector(".join-cancel").onclick = close;
...
function render() {
  ...
  const playerJoinModal = !me && joinModalOpen ? `<div class="join-modal-backdrop player-join-modal-backdrop"><section class="join-modal" role="dialog" aria-modal="true" aria-labelledby="player-join-title"><div class="phase">LIVE MATCHDAY</div><h2 id="player-join-title">${esc(f.home?.name)} <i>v</i> ${esc(f.away?.name)}</h2><p class="muted">Join this live match and make your calls as it unfolds.</p><label class="join-modal-name">Your name<input type="text" maxlength="20" autocomplete="name" value="${esc(pendingName)}" placeholder="Enter your name" data-player-join-name></label><div class="join-modal-actions"><button type="button" class="join-cancel" data-player-join-cancel>Not now</button><button type="button" class="join-confirm" data-player-join-confirm>Join game</button></div></section></div>` : "";
